    ...
    new mongodb.MongoClient(self.settings.url, validOptions).connect(function(err, client) {
      if (!err) {
        ...
        return urlParser(self.settings.url, self.settings, function(err, url) {
          var db = (url.dbName || self.settings.database);
          self.tenant = self.settings.tenant || db;
          self.tenantPrefix = self.settings.tenantPrefix || '';
          self.db = client.db(db, url.db_options || self.settings);
 ...
 * @returns {*}
 */
MongoDB.prototype.collection = function(model, options) {
  if (!this.db) {
    throw new Error(g.f('{{MongoDB}} connection is not established'));
  }
  var collectionName = this.collectionName(model);
  var tenant = (options && options.accessToken && options.accessToken.tenant) || this.tenant;
 ...
 */
MongoDB.prototype.execute = function(model, command, options) {
  var self = this;
  ...
  function doExecute() {
    ...
    try {
      var collection = self.collection(model, options);
    } catch (err) {
      debug('Error: ', err);
      callback(err);
      return;
...
MongoDB.prototype.create = function(model, data, options, callback) {
  ...
  this.execute(model, 'insert', options, data, { safe: true }, function(err, result) {
    if (self.debug) {
...
MongoDB.prototype.save = function(model, data, options, callback) {
  ...
  this.execute(model, 'save', options, data, {}, function(err, result) {
    if (!err) {
...
MongoDB.prototype.exists = function(model, id, options, callback) {
  ...
  this.execute(model, 'findOne', options, { _id: id }, function(err, data) {
    if (self.debug) {
...
MongoDB.prototype.find = function find(model, id, options, callback) {
  ...
  this.execute(model, 'findOne', options, { _id: oid }, function(err, data) {
    if (self.debug) {
...
MongoDB.prototype.updateOrCreate = function updateOrCreate(model, data, options, callback) {
  ...
  this.execute(model, 'findAndModify', options, {
    _id: oid,
  }, [
    ['_id', 'asc'],
...
MongoDB.prototype.replaceOrCreate = function(model, data, options, cb) {
  ...
  delete data[idName];
  this.replaceWithOptions(model, oid, data, { upsert: true }, options, cb);
};
...
MongoDB.prototype.destroy = function destroy(model, id, options, callback) {
  ...
  id = self.coerceId(model, id);
  this.execute(model, 'remove', options, { _id: id }, function(err, result) {
    if (self.debug) {
...
MongoDB.prototype.all = function all(model, filter, options, callback) {
  ...
  if (fields) {
    this.execute(model, 'find', options, query, fields, processResponse);
  } else {
    this.execute(model, 'find', options, query, processResponse);
  }
...
MongoDB.prototype.destroyAll = function destroyAll(model, where, options, callback) {
  ...
  this.execute(model, 'remove', options, where || {}, function(err, info) {
...
MongoDB.prototype.count = function count(model, where, options, callback) {
  ...
  where = self.buildWhere(model, where);
  this.execute(model, 'count', options, where, function(err, count) {
    if (self.debug) {
...
MongoDB.prototype.replaceById = function replace(model, id, data, options, cb) {
  ...
  var oid = this.coerceId(model, id);
  this.replaceWithOptions(model, oid, data, { upsert: false }, options, cb);
};
 ...
 */
MongoDB.prototype.replaceWithOptions = function(model, id, data, update, options, cb) {
  var self = this;
  var idName = self.idName(model);
  delete data[idName];
  this.execute(model, 'update', options, { _id: id }, data, update, function(err, info) {
...
MongoDB.prototype.updateAttributes = function updateAttrs(model, id, data, options, cb) {
  ...
  this.execute(model, 'findAndModify', options, { _id: oid }, [
    ['_id', 'asc'],
...
MongoDB.prototype.update =
  MongoDB.prototype.updateAll = function updateAll(model, where, data, options, cb) {
    ...
    this.execute(model, 'update', options, where, data, { multi: true, upsert: false },
      function(err, info) {
        if (err) return cb && cb(err);
  ...
  if (self.db) {
    ...
    async.each(models, function(model, modelCallback) {
      ...
      async.each(indexList, function(index, indexCallback) {
        ...
        self.collection(model, cbOptions).createIndex(index.fields || index.keys, index.options, indexCallback);
      }, modelCallback);
    }, cb);
  } else {
    self.dataSource.once('connected', function() {
