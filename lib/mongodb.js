  ...
  if (self.db) {
    ...
    new mongodb.MongoClient(self.settings.url, validOptions).connect(function(
      ...
      return urlParser(self.settings.url, self.settings, function(err, url) {
        ...
        const dbName = (url.dbName || self.settings.database);
        self.tenant = self.settings.tenant || db;
        self.tenantPrefix = self.settings.tenantPrefix || '';
        self.db = client.db(dbName, url.db_options || self.settings);
 ...
 * @returns {*}
 */
MongoDB.prototype.collection = function(modelName, options) {
  if (!this.db) {
    throw new Error(g.f('{{MongoDB}} connection is not established'));
  }
  const collectionName = this.collectionName(modelName);
  const tenant = (options && options.accessToken && options.accessToken.tenant) || this.tenant;
  return this.client.db(this.tenantPrefix + tenant).collection(collectionName);
};
 ...
 */
MongoDB.prototype.execute = function(modelName, command, options) {
  const self = this;
  ...
  function doExecute() {
    ...
    try {
      collection = self.collection(modelName, options);
    } catch (err) {
      debug('Error: ', err);
      callback(err);
      return;
...
MongoDB.prototype.coerceId = function(modelName, id, options) {
  // See https://github.com/strongloop/loopback-connector-mongodb-mt/issues/206
...
MongoDB.prototype.create = function(modelName, data, options, callback) {
  ...
  this.execute(modelName, 'insertOne', options, data, {safe: true}, function(err, result) {
    if (self.debug) {
...
MongoDB.prototype.save = function(modelName, data, options, callback) {
  ...
  this.execute(modelName, 'updateOne', options, {_id: oid}, {$set: data}, {upsert: true}, function(err, result) {
    if (!err) {
...
MongoDB.prototype.exists = function(modelName, id, options, callback) {
  ...
  this.execute(modelName, 'findOne', options, {_id: id}, function(err, data) {
    if (self.debug) {
...
MongoDB.prototype.find = function find(modelName, id, options, callback) {
  ...
  this.execute(modelName, 'findOne', options, {_id: oid}, function(err, data) {
    if (self.debug) {
...
MongoDB.prototype.replaceOrCreate = function(modelName, data, options, cb) {
  ...
  delete data[idName];
  this.replaceWithOptions(modelName, oid, data, {upsert: true}, options, cb);
};
...
MongoDB.prototype.destroy = function destroy(modelName, id, options, callback) {
  ...
  id = self.coerceId(modelName, id, options);
  this.execute(modelName, 'deleteOne', options, {_id: id}, function(err, result) {
    if (self.debug) {
...
MongoDB.prototype.all = function all(modelName, filter, options, callback) {
  ...
  if (fields) {
    ...
    this.execute(modelName, 'find', options, query, findOpts, processResponse);
  } else {
    this.execute(modelName, 'find', options, query, processResponse);
  }
...
MongoDB.prototype.destroyAll = function destroyAll(
  ...
  this.execute(modelName, 'deleteMany', options, where || {}, function(err, info) {
...
MongoDB.prototype.count = function count(modelName, where, options, callback) {
  ...
  this.execute(modelName, method, options, where, function(err, count) {
    if (self.debug) {
...
MongoDB.prototype.replaceById = function replace(modelName, id, data, options, cb) {
  ...
  const oid = this.coerceId(modelName, id, options);
  this.replaceWithOptions(modelName, oid, data, {upsert: false}, options, function(
    err,
    data,
  ) {
    cb(err, data);
 ...
 * @param {Object} update The options you want to pass for update, e.g, {upsert: true}
 * @callback {Function} [cb] Callback function
 */
MongoDB.prototype.replaceWithOptions = function(modelName, id, data, update, options, cb) {
  const self = this;
  const idName = self.idName(modelName);
  delete data[idName];
  data = self.toDatabase(modelName, data);
  this.execute(modelName, 'replaceOne', options, {_id: id}, data, update, function(
    err,
    info,
  ) {
...
MongoDB.prototype.disconnect = function(cb) {
  ...
  at MongoDB.disconnect (node_modules/loopback-connector-mongodb-mt/lib/mongodb.js:1660:13)
  */
  /*
  if (this.db) {
    this.db.unref();
 ...
 * @param {Function} [cb] The callback function
 */
MongoDB.prototype.autoupdate = function(models, cbOptions, callback) {
  ...
  if (self.db) {
      ...
      function(modelName, modelCallback) {
        ...
        async.each(
          ...
          function(index, indexCallback) {
            ...
            self
              .collection(modelName, cbOptions)
              .createIndex(
    ...
    self.dataSource.once('connected', function() {
      self.autoupdate(models, cbOptions, cb);
    });
  }
};
 ...
 * @param {Function} [cb] The callback function
 */
MongoDB.prototype.automigrate = function(models, cbOptions, callback) {
  if (!Array.isArray(models)) models = [models];
  ...
  if (self.db) {
    ...
    self.dataSource.once('connected', function() {
      self.automigrate(models, cbOptions, cb);
    });
  }
};
...
function optimizedFindOrCreate(modelName, filter, data, options, callback) {
  ...
  this.collection(modelName, options).findOneAndUpdate(
    query,
    {$setOnInsert: data},
